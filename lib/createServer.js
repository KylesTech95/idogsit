...
const dir = !existsSync(path.resolve(__dirname,'servers')) ? mkdirSync(path.resolve(__dirname,'servers')) : readdirSync(path.resolve(__dirname,'servers'));
...
num <= maximumServers && Math.abs((dir.length||0) - num) < maximumServers ? duplicateServerFn(num,dir,jsonObj) : console.log('Maximum server limit: ' + maximumServers);
